    ...
    <main className="min-h-screen bg-graphite-950 text-white">
        ...
        <div className="subpage-container relative z-10">
          <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-10 lg:gap-16 items-end">
            <div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-8"><a href="#kapcsolat" className="signal-button-primary">Diagnózis indítása <span aria-hidden="true">↗</span></a><a href="#scope" className="signal-button-secondary">Mit vizsgálunk?</a></div>
            </div>
            <aside className="tech-status-panel">
              <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4 mb-5"><span className="text-xs uppercase tracking-[0.18em] text-white/45">Signal check</span><span className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-signal-400"><span className="signal-dot" />diagnostic</span></div>
              <div className="space-y-3">{['Problem','Cause','Priority','Action'].map((item,index) => <div key={item} className="flex items-center justify-between gap-5 py-2 border-b border-white/[0.06] last:border-0"><span className="text-sm text-white/40">{String(index+1).padStart(2,'0')}</span><span className="text-sm text-white">{item}</span></div>)}</div>
      ...
      <section className="hightech-section bg-graphite-900 relative overflow-hidden">
        ...
        <div className="hightech-container relative grid lg:grid-cols-[0.7fr_1.3fr] gap-10 lg:gap-16 items-start">
          <div className="lg:sticky lg:top-28"><div className="text-xs uppercase tracking-[0.18em] text-white/30 mb-4">Symptoms / signals</div><h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-light tracking-editorial leading-[0.94] text-balance">Amit általában <span className="text-signal-400">először észreveszel.</span></h2></div>
          <div className="grid sm:grid-cols-2 gap-4 md:gap-5">{symptoms.map((symptom,index) => <div key={index} className="tech-card-dark p-5 sm:p-6"><div className="text-[10px] uppercase tracking-[0.18em] text-signal-400 mb-7">SIG/{String(index+1).padStart(2,'0')}</div><p className="text-white/65 font-light text-base sm:text-lg leading-relaxed">{symptom}</p></div>)}</div>
        </div>
      </section>
      ...
      <section className="hightech-section bg-graphite-950 relative overflow-hidden">
        ...
        <div className="hightech-container relative grid lg:grid-cols-[0.75fr_1.25fr] gap-10 lg:gap-16">
          <div><div className="signal-kicker mb-5"><span className="signal-dot" />Root causes</div><h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-light tracking-editorial leading-[0.94] text-balance">{causes.title}</h2></div>
      ...
      <section id="scope" className="hightech-section bg-graphite-900 scroll-mt-24 relative overflow-hidden">
        ...
        <div className="hightech-container relative">
          <div className="max-w-3xl mb-10 md:mb-14"><div className="text-xs uppercase tracking-[0.18em] text-signal-400 mb-4">Diagnostic scope</div><h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-light tracking-editorial leading-[0.94] text-balance mb-5">{auditScope.title}</h2><p className="text-lg text-white/55 font-light">Nem checklista kedvéért mérünk. A cél, hogy rangsorolható legyen, melyik beavatkozásnak van üzleti értelme.</p></div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">{auditScope.items.map((item,index) => <div key={index} className="tech-card-dark p-6 min-h-[160px] flex flex-col"><div className="text-[10px] uppercase tracking-[0.18em] text-signal-400 mb-8">CHK/{String(index+1).padStart(2,'0')}</div><p className="text-white/65 font-light leading-relaxed mt-auto">{item}</p></div>)}</div>
        </div>
      </section>
      ...
      <section className="hightech-section bg-graphite-950">
        <div className="hightech-container grid lg:grid-cols-[0.75fr_1.25fr] gap-10 lg:gap-16 items-start">
          <div className="lg:sticky lg:top-28"><div className="text-xs uppercase tracking-[0.18em] text-signal-400 mb-4">Output</div><h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-light tracking-editorial leading-[0.94] text-balance">{deliverables.title}</h2></div>
      ...
      <section id="kapcsolat" className="hightech-section bg-signal-400 text-graphite-950 scroll-mt-24"><div className="hightech-container"><div className="grid lg:grid-cols-[1fr_0.9fr] gap-9 lg:gap-14 items-end mb-10"><div><div className="text-xs uppercase tracking-[0.2em] opacity-55 mb-4">Next step</div><h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-light tracking-editorial leading-[0.95] text-balance">{cta.title}</h2></div><p className="text-base sm:text-lg opacity-70 font-light leading-relaxed">{cta.subtitle}</p></div><div className="grid sm:grid-cols-3 gap-3 sm:gap-4"><a href="https://wa.me/36202826843?text=Szia%2C%20a%20weboldalam%20m%C5%B1k%C3%B6d%C3%A9s%C3%A9t%20szeretn%C3%A9m%20%C3%A1tn%C3%A9zetni." target="_blank" rel="noopener noreferrer" className="bg-graphite-950 text-white p-5 sm:p-6 flex items-center gap-4 hover:bg-graphite-900 transition-colors"><MessageCircle className="w-5 h-5 text-signal-400" /><div><div className="font-medium">WhatsApp</div><div className="text-xs text-white/45">Gyors projektindítás</div></div></a><a href="mailto:hello@leventestudio.app?subject=Weboldal%20diagn%C3%B3zis%20%C3%A9s%20rendszerterv" className="bg-graphite-950 text-white p-5 sm:p-6 flex items-center gap-4 hover:bg-graphite-900 transition-colors"><Mail className="w-5 h-5 text-signal-400" /><div><div className="font-medium">Email</div><div className="text-xs text-white/45">hello@leventestudio.app</div></div></a><a href="tel:+36202826843" className="bg-graphite-950 text-white p-5 sm:p-6 flex items-center gap-4 hover:bg-graphite-900 transition-colors"><Phone className="w-5 h-5 text-signal-400" /><div><div className="font-medium">Telefon</div><div className="text-xs text-white/45">+36 20 282 6843</div></div></a></div></div></section>

      {relatedProblems && relatedProblems.length > 0 && <section className="hightech-section bg-graphite-950"><div className="hightech-container max-w-5xl"><div className="text-xs uppercase tracking-[0.18em] text-signal-400 mb-4">Related signals</div><h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-light tracking-editorial leading-[0.94] text-balance mb-10 md:mb-12">Kapcsolódó problémák</h2><div className="grid sm:grid-cols-2 gap-4 md:gap-5">{relatedProblems.map((problem,index) => <a key={index} href={problem.url} className="tech-card-dark p-6 sm:p-7 group"><div className="flex items-start justify-between gap-4"><div><div className="text-[10px] uppercase tracking-[0.18em] text-signal-400 mb-5">REL/{String(index+1).padStart(2,'0')}</div><h3 className="font-serif text-2xl sm:text-3xl font-light tracking-editorial text-white mb-2">{problem.title}</h3><p className="text-white/50 font-light text-sm leading-relaxed">{problem.description}</p></div><ArrowRight className="w-5 h-5 text-signal-400 group-hover:translate-x-1 transition-all flex-shrink-0" /></div></a>)}</div></div></section>}
    </main>
  );
}
