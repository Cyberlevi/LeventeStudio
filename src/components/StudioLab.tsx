    ...
    <section id="lab" className="scroll-mt-20 bg-ivory-100 px-5 py-20 text-graphite-950 sm:px-6 md:py-24 lg:px-8">
      <div className="mx-auto max-w-[86rem]">
        <div className="mb-10 grid gap-6 lg:grid-cols-2 lg:items-end">
          ...
          <p className="max-w-xl text-base leading-relaxed text-graphite-600 lg:justify-self-end">Négy saját vagy családi szolgáltatás, négy különböző feladat. Ezeken keresztül mutatjuk meg a munkánkat és a megoldásainkat.</p>
        </div>
        <div className="grid gap-6 md:grid-cols-2">
          {referenceProjects.map((project) => {
            const primaryUrl = project.href ?? project.liveUrl ?? '#lab';
            const primaryIsExternal = !project.href && Boolean(project.liveUrl);
            return (
              <article key={project.id} className="overflow-hidden rounded-xl border border-graphite-950/10 bg-white">
                <a href={primaryUrl} target={primaryIsExternal ? '_blank' : undefined} rel={primaryIsExternal ? 'noopener noreferrer' : undefined} className="group block overflow-hidden border-b border-graphite-950/10" aria-label={primaryIsExternal ? `${project.name} élő weboldal` : `${project.name} projektbemutatás`}>
                ...
                <div className="flex flex-col p-6 sm:p-8">
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-graphite-950/10 pt-5 text-sm">
                    <a href={primaryUrl} target={primaryIsExternal ? '_blank' : undefined} rel={primaryIsExternal ? 'noopener noreferrer' : undefined} className="inline-flex items-center gap-2 font-medium hover:underline">{primaryIsExternal ? 'Élő weboldal' : 'Projekt bemutatása'} <ArrowUpRight size={15} aria-hidden="true" /></a>
                    {project.href && project.liveUrl && <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-graphite-500 hover:text-graphite-950">Élő weboldal <span className="sr-only">(új lapon)</span> ↗</a>}
                  </div>
